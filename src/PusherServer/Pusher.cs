...
namespace PusherServer
{
    ...
    public class Pusher : IPusher
    {
        ...
        public static String LIBRARY_NAME
        {
            get
            {
                //return typeof(Pusher).GetTypeInfo().GetCustomAttribute<AssemblyProductAttribute>().Product;
                return "Pusher";
            }
        }
        ...
        private BatchTriggerBody CreateBatchTriggerBody(Event[] events)
        {
            ...
            var batchEvents = new List<BatchEvent>(events.Length);

            foreach(var e in events)
            {
                batchEvents.Add(new BatchEvent {
                    name = e.EventName,
                    channel = e.Channel,
        ...
        public IGetResult<T> Get<T>(string resource, object parameters)
        {
            ...
            return new GetResult<T>(taskCompletionSource.Task.Result, _options.JsonDeserializer);
        }
